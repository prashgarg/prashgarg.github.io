// Desk-rebuild capture: idle + lean (3D) + no-regression overlay check.
// Writes to shots/goal0529/dr-*.png so baselines (r2-*, pw2-*) stay intact.
...
// A) idle + lean
{
  ...
  await p.screenshot({ path: `${DIR}/dr-idle.png` });
  await p.mouse.move(700, 700); await sleep(1800);
  await p.screenshot({ path: `${DIR}/dr-lean.png` });
  await ctx.close();
}
